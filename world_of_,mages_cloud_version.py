...
class Mage:
    ...
    def postac(self):
        print(f"Your name is {self.name} you are a {self.typ} aspect mage, user of {self.weapon}")
...
def login():
    ...
    if start == "1":
        ...
        username = input("Set your usename:")
        password = input("Set your password:")
        try:
            mycursor.execute(f"INSERT INTO users Values(null,'{username}',sha2('{password}',256))")
            mydb.commit()
        ...
        sys.exit()
    mycursor.execute(f"SELECT * FROM users WHERE name_user = '{username}' and password_user = sha2('{password}', 256)")
...
def starting():
    global user_id
    mycursor.execute(f"select * from mage where user_id = {user_id}")
    mage = mycursor.fetchall()
    ...
    if not mage:
        ...
        if choice == "1":
            ...
            character.postac()
            mycursor.execute(f"INSERT INTO mage VALUES(null, {user_id}, '{name}', '{element}', '{weapon}')")
            mydb.commit()
            mycursor.execute(f"INSERT INTO equipment VALUES(null,{user_id}, 'basic robe', 'basic ring', "
                             f"'basic ring', 'basic necklace', 100, 0, 0, 0, 0)")
            mydb.commit()
            print("1-to continue")
            tak = input()
            if tak == "1":
            ...
            def randcha():
                ...
                if czy == "1":
                    ...
                    mycursor.execute(f"INSERT INTO mage VALUES(null, {user_id}, 'morgan', '{element}', '{weapon}')")
                    mydb.commit()
                    mycursor.execute(f"INSERT INTO equipment VALUES(null,{user_id}, "
                                     f"'Basic Robe', 'Basic Ring', 'Basic Ring', 'Basic Necklace', 100, 0, 0, 0, 0)")
                    mydb.commit()
                    eq = Things()
                    eq.save()
                    daily_shop()
...
def main():
    global user_id
    print("#####################################")
    mycursor.execute(f"SELECT element, weapon, mage_name FROM mage WHERE user_id = {user_id}")
    tab = mycursor.fetchall()
    tab = list(tab[0])
    character = Mage(tab[0], tab[1], tab[2])
    mycursor.execute(f"SELECT robe, ring1, ring2, necklace, gold, potion, tier1, "
                     f"tier2, tier3 from equipment WHERE user_id = {user_id}")
        ...
        if re.match(character.weapon, weapons[_][0]):
            while enn.hp > 0 and stats.hp > 0:
                ...
                if who > weapons[_][1]:
                    ...
                    print("Your mana: " + str(stats.mana) + "/{0}".format(maxmp))
                    print("Your hp potion: {0}".format(eq.potion))
                    print("goblin hp: {0}".format(enn.hp))
                    time.sleep(1)
